...
- Frontend pages beyond 1–4 and 7–10 (WP-12/WP-13 delivered those — see above), including page 5 (super-admin dashboard), page 6 (role-shaped dashboard — WP-12's placeholder `/dashboard` proves the wiring but isn't the real page), and page 15 (shifts) — WP-09's Section 19 entry named it, but frontend foundation didn't exist until WP-12
...
- `pending_reimbursements`, `last_payroll_run`, `latest_payslip_status`, `team_task_load`, `assigned_open_tasks` on the WP-11 dashboard — hardcoded to `0`/`None`; the underlying `payroll_runs`/reimbursement/`tasks` tables don't exist yet
...
**Not delivered:** a public `industry_presets` list endpoint for the registration page's industry field (still free text) — this is a backend gap, not a frontend one, and adding a new route is out of scope for "build the four pages"; noted for whoever picks up a future platform/company-onboarding pass.
