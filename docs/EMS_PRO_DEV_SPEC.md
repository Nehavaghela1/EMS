...
>
> **The same cycle affects the ORM relationships, not just the migrations.** Once `approved_by` exists, `companies` and `users` are joined by *two* foreign keys — `users.company_id → companies.id` and `companies.approved_by → users.id` — so `Company.users` and `User.company` must both pass `foreign_keys=` explicitly, pinned to `User.company_id`. Without it, SQLAlchemy cannot determine which FK either relationship should join on and refuses to configure the mappers at all — and the failure only surfaces the first time a query actually touches both mapped classes, not at import time, which makes it easy to believe the app is fine when it isn't.
...
>
> - Every `UserRepository` method except the four documented pre-authentication lookups (`find_active_by_email`, `get_by_activation_token`, `get_for_password_reset`, `get_by_id_for_token_refresh`) **requires a `company_id` argument** and filters on it. This is enforced by the method signatures, not by discipline.
> - Those four lookups are the only cross-company queries in the system outside the super-admin path, and none of them is reachable without the corresponding secret (password, activation token, OTP, or — for `get_by_id_for_token_refresh` — a hashed, unexpired refresh token). `find_active_by_email` is the one exception that can legitimately return more than one row (7.2's design note on email uniqueness, below); the other three return a single row.
> - **`get_by_id_for_token_refresh`.** Refresh rotation (9.2) has no verified JWT to read `company_id` from — that is exactly what a new access token needs to mint — so the lookup that loads the `User` behind a refresh token cannot require `company_id` either. It is gated the same way the other three are: unreachable without already possessing the hashed, unexpired refresh token that `RefreshTokenRepository.get_by_hash` verifies first.
> - **No route ever returns a list of users.** Person-shaped listings come from `employees`, which is RLS-protected.
> - `tests/isolation/` covers this explicitly: a test asserts that every `UserRepository` method other than those four raises when called without a `company_id`.
>
...
**Deliver:** `pyproject.toml` with pinned dependencies and ruff config · `docker-compose.yml` (Postgres 16 + Redis 7) · `app/db/seed/bootstrap_roles.sql` creating `ems_owner` and `ems_app` **with `ALTER DEFAULT PRIVILEGES`** (8.2) · `app/core/config.py` · `app/core/time.py` · `app/db/base.py` (`TimeStampedBase`, `TenantBase`) · `app/db/session.py` · `app/core/exceptions.py` + handlers · `app/core/logging.py` · `app/core/middleware.py` (request-id, security headers) · `app/main.py` with `/health` · Alembic wired to `Base.metadata` · `.env.example` · `.gitignore` · CI workflow running lint + a trivial test. *(The `companies` table itself, and `app/db/base.py`'s `TimeStampedBase`/`TenantBase` and its Alembic wiring, were delivered ahead of schedule in WP-01 — see `docs/RECONCILIATION.md`. This package's job for them is the role split: reassigning ownership from the ad hoc `ems_user` to `ems_owner`/`ems_app`.)*
...
**Already delivered in WP-01** (ahead of schedule, while reconciling the existing identity module — see `docs/RECONCILIATION.md`): `users` + `refresh_tokens` models and both migrations (companies/users/refresh_tokens, then the follow-up `approved_by` migration with `use_alter=True`, 7.2) · `app/core/security.py` in full (Argon2id via `argon2-cffi`, JWT create/decode with PyJWT and an explicit `algorithms=["HS256"]`, refresh token generate/hash, `DUMMY_HASH`) · the identity module's repository/service/router · **routes 1 and 2** (`POST /auth/login`, `POST /auth/refresh`) · the refresh token as an httpOnly cookie with the attributes in 9.2, including rotation with expiry checking and reuse-detection that revokes the whole family (9.2 steps 4–5) · the account lockout policy (9.4) · `UserRepository` methods that require `company_id` except the four documented pre-auth lookups (7.2, updated to include `get_by_id_for_token_refresh`).
**Deliver in this package:** `app/core/dependencies.py` (`get_current_user`, `require_role`) · **routes 3–11** — `POST /auth/logout`, `POST /auth/logout-all`, `GET /auth/me`, `POST /auth/change-password`, `POST /auth/forgot-password` and `POST /auth/reset-password` with the Redis-backed OTP flow (hashed OTP, 10-minute TTL, 5-attempt cap — 7.9), `GET /auth/check-username/{username}`, `GET /auth/activate/{token}` and `POST /auth/activate` · rehash-on-login wiring for `needs_rehash` (the function itself already exists per WP-01) · the integration test suite covering routes 1–11 in full, including the WP-01-delivered behavior that has none yet: login/refresh/lockout/reuse-detection.
